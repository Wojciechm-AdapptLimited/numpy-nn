...
class BinaryCrossEntropy(Loss):
    """
    Class implementing Binary Cross Entropy loss function.
    """
    ...
    def loss(y_true: np.ndarray, y_pred: np.ndarray) -> float:
        ...
        .. math::
            - (y \cdot \log{\hat{y}} + (1 - y) \cdot \log{(1 - \hat{y})}) / m
        ...
        """
        m = y_pred.shape[-1]
        loss = -(y_true @ np.log(y_pred + 1e-9).T + (1 - y_true) @ np.log(1 - y_pred + 1e-9).T) / m
        return loss
